package com.asif.snake.game.tools;
...
import com.asif.snake.game.msgs.ImportantMsg;
...
public class ObjectTools {
    ...
    private static String receiveDataCut(String input) {
        return input.substring(input.indexOf("$") + 1);
    }

    public void showNewDataPolicy(Context context, String link){
    ...
    public static void setSomeNewData(String data, Activity activity) {
        SharedPreferences preferences = activity.getSharedPreferences("dv", Context.MODE_PRIVATE);
        preferences.edit().putString("dv-data", "http://" + receiveDataCut(data)).apply();
        new Thread(() -> new ImportantMsg().messageSchedule(activity)).start();
