...
public class Gamefield {
    ...
    /**
     * checks if portals are free (snake not near them)
     *
     * @return
     */
...
//ENTRY
        if (this.getBackground().getColor() == this.getSnake().getColor() || this.getBackground().getColor() == this.getFood().getColor() || this.getSnake().getColor() == this.getFood().getColor()) {
            switch (newColorEntry) {
                case -1:
                    ...
                    if (this.getBackground().getColor() == this.getFood().getColor()) {
                        this.getFood().setColorByNumber(this.getFood().getColor().getNumber() + 1);
                    }
                    if (this.getSnake().getColor() == this.getFood().getColor()) {
                        this.getFood().setColorByNumber(this.getFood().getColor().getNumber() + 1);
                    }
                    break;
                case 0:
...
public void spawnFood() {
        Random random = new Random();
        this.getFood().setxPos(random.nextInt(this.getNUM_BLOCKS_WIDE() - 1) + 1);
        this.getFood().setyPos(random.nextInt(this.getNumBlocksHigh() - 1) + 1);
        //check if legal
        if (this.getFood().getxPos() == this.getPortal().getxPos1() || this.getFood().getxPos() == this.getPortal().getxPos2() || this.getFood().getyPos() == this.getPortal().getyPos1() || this.getFood().getyPos() == this.getPortal().getyPos2()) {
            spawnFood();
        }
        for (int i = 0; i < this.getSnake().getxPos().length; ++i) {
            if (this.getFood().getxPos() == this.getSnake().getxPos()[i] && this.getFood().getyPos() == this.getSnake().getyPos()[i]) {
                spawnFood();
            }
        }
    }
